...
for i, b in enumerate(bin_labels):
    with h5py.File(data_dir+selection+'_'+angle+'_sfr_data.h5') as f:
        no_gals = f[gals+'_no_gals_'+b].value
        mean_tukey = f[gals+'_jk_'+b].value
...
# plot sSFR:
for i, b in enumerate(bin_labels):
    with h5py.File(data_dir+selection+'_'+angle+'_ssfr_data.h5') as f:
        no_gals = f[gals+'_no_gals_'+b].value
        mean_tukey = f[gals+'_jk_'+b].value
        tukey = f[gals+'_tukey_'+b].value
        cv_err = f[gals+'_cv_err_'+b].value
    ...
    ax[1].fill_between(rplot, tukey - cv_err, tukey + cv_err, color=colors[i], alpha=0.25)
if gals == 'all':
    ax[1].set_xlim(0, 3.5)
    ax[1].set_ylim(-12.0, -9.5)
else:
    ax[1].set_xlim(0., 2)
    ax[1].set_ylim(-12.5, -10.)
...
# plot HI:
for i, b in enumerate(bin_labels):
    with h5py.File(data_dir+selection+'_'+angle+'_h1_data.h5') as f:
        no_gals = f[gals+'_no_gals_'+b].value
        mean_tukey = f[gals+'_jk_'+b].value
        tukey = f[gals+'_tukey_'+b].value
        cv_err = f[gals+'_cv_err_'+b].value
    ...
    ax[2].fill_between(rplot, tukey - cv_err, tukey + cv_err, color=colors[i], alpha=0.25)
ax[2].set_ylim(4.5,8.)
ax[2].set_xlim(0, 5)
...
# plot fmol:
for i, b in enumerate(bin_labels):
    with h5py.File(data_dir+selection+'_'+angle+'_fmol_data.h5') as f:
        no_gals = f[gals+'_no_gals_'+b].value
        mean_tukey = f[gals+'_jk_'+b].value
        tukey = f[gals+'_tukey_'+b].value
        cv_err = f[gals+'_cv_err_'+b].value
...
plt.savefig(plot_dir+selection+'_'+angle+'_all_radial_cv.png')
plt.show()
plt.clf()
